...
from cacheops import cached_as, invalidator
...
class SignalsTests(BaseTestCase):
    ...
    def test_invalidation_signal(self):
        ...
        invalidator.invalidate_model(Post)
...
class NoInvalidationTests(BaseTestCase):
    ...
    def test_context_manager(self):
        def invalidate(post):
            with invalidator.no_invalidation:
                invalidator.invalidate_obj(post)
        self._template(invalidate)

    def test_decorator(self):
        self._template(invalidator.no_invalidation(invalidator.invalidate_obj))
