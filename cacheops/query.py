...
def cache_thing(cache_key, data, cond_dnfs, timeout):
    ...
    """
    assert not transaction_state.allows_caching()
...
def cached_as(*samples, **kwargs):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            if transaction_state.allows_caching() or not settings.CACHEOPS_ENABLED:
...
class QuerySetMixin(object):
    ...
    def iterator(self):
        ...
        if not self._cacheprofile or 'fetch' not in self._cacheprofile['ops'] \
                or transaction_state.allows_caching() or not settings.CACHEOPS_ENABLED:
